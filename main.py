...
def process_document(uploaded_file: UploadedFile) -> list[Document]:
    ...
    # Delete temp file safely after usage
    try:
        if os.path.exists(temp_file_path):
            os.unlink(temp_file_path)
    except PermissionError:
        st.error(f"Permission denied to delete the file {temp_file_path}")
    except Exception as e:
        st.error(f"Failed to delete temporary file: {e}")
...
if __name__ == "__main__":
    ...
    with st.sidebar:
        ...
        uploaded_file = st.file_uploader(
            "**📑 Upload PDF files for QA**", type=["pdf"], accept_multiple_files=False
        )
    ...
    st.header("RAG-QA-FINANCIAL-DATA")
